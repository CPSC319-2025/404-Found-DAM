...
namespace Core.Services
{
    public class PaletteService : IPaletteService
    {
      ...
      public async Task<RemoveTagsResult> RemoveTagsFromAssetsAsync(List<int> blobIds, List<int> tagIds)
        {
            ...
            foreach (int blobId in blobIds)
            {
                foreach (int tagId in tagIds)
                {
                    ...
                    if (exists)
                    {
                        bool removed = await _paletteRepository.RemoveAssetTagFromDb(blobId, tagId);
                        if (removed)
                        {
